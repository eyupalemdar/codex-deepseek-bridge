...
try {
    ...
    if((Get-Content -Raw (Join-Path $bin 'test-codex-provider.cmd')) -notmatch 'call codex'){throw 'Launcher does not preserve batch environment with CALL.'}
    ...
    [ordered]@{status='passed';install=$true;catalog=$true;launcher_call_fix=$true;project_registration=$true;broker_validation=$true}|ConvertTo-Json
}
finally {
